...
export class FPNumber {
  ...
  public format(
    dp = FPNumber.DEFAULT_DECIMAL_PLACES,
    format?: BigNumber.Format,
  ): string {
    const value = this.value.div(10 ** this.precision);
    if (value.isZero()) {
      // @ts-expect-error Type 'Format' is not assignable to type 'number'.
    ...
    if (formatted.isZero()) {
      ...
      formatted = new BigNumber(
        value.toFormat().replace(/(0\.0*[1-9])([0-9]*)/, '$1'),
      );
    }
    // @ts-expect-error Type 'Format' is not assignable to type 'number'.
  ...
  public toLocaleString(): string {
    ...
    return decimal
      ? integer.concat(FPNumber.DELIMITERS_CONFIG.decimal, decimal)
      : integer;
  }
  ...
  public add(target: FPNumber): FPNumber {
    return new FPNumber(
      this.value
        .plus(equalizedBN(target, this.precision))
  ...
  public sub(target: FPNumber): FPNumber {
    return new FPNumber(
      this.value
        .minus(equalizedBN(target, this.precision))
        .dp(0, FPNumber.DEFAULT_ROUND_MODE),
      this.precision,
    );
  }
